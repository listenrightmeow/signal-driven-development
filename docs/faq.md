...
- **Pass 1**: 2-4 hours for initial extraction and gap report
- **Pass 2**: 1-2 hours for resolution and re-evaluation
- **Pass 3**: 30-60 minutes for final convergence check
...
Investigate the root cause. Revert the resolution that caused divergence if needed. The convergence invariant is non-negotiable.

## Do I need all four templates for every domain?

The **domain specification** and **gap report** are essential. The **gap resolution log** is strongly recommended -- it captures the rationale that you will forget in two weeks. The **architecture palette** is optional but valuable for complex domains where visual patterns reveal issues that text cannot.
...
They are complementary. EventStorming is a discovery technique -- it helps you find domain events and command flows through collaborative workshops. SDD is a convergence technique -- it takes a domain specification (however you produced it) and drives it to completeness through diagnostic passes. You could use EventStorming for discovery and SDD for convergence.

## Can I contribute a worked example?

Yes. See [CONTRIBUTING.md](../CONTRIBUTING.md) for guidelines on submitting anonymized domain analyses.
